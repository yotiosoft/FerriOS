...
pub fn init() {
    ...
    crate::debug!("cpu_ptr: {:#x}", cpu_ptr);
    crate::debug!("kernel_syscall_rsp offset: {}", core::mem::offset_of!(Cpu, kernel_syscall_rsp));
    KernelGsBase::write(VirtAddr::new(cpu_ptr));
    crate::debug!("KernelGsBase written: {:#x}", KernelGsBase::read().as_u64());
}
